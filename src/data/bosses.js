...
async function tryGiveRole(guild, userId, roleId) {
  try {
    ...
    if (!botMember.permissions.has(PermissionsBitField.Flags.ManageRoles)) return { ok: false, reason: "Missing Manage Roles." };
    const role = guild.roles.cache.get(roleId);
    if (!role) return { ok: false, reason: "Role not found." };
    const botTop = botMember.roles.highest?.position ?? 0;
    if (botTop <= role.position) return { ok: false, reason: "Bot role is below target role." };
    const member = await guild.members.fetch(userId);
    await member.roles.add(roleId);
    return { ok: true };
  } catch {
    return { ok: false, reason: "Role add rejected." };
  }
}
...
async function runBoss(channel, boss) {
  try {
    ...
    for (let i = 0; i < boss.def.rounds.length; i++) {
      ...
      if (r.type === "coop_block") {
        ...
        const msg = await channel.send({
          content: `🛡️ COOP BLOCK: ${Math.round((r.windowMs || 5000) / 1000)}s • need ${boss.activeAction.requiredPresses} players`,
        ...
        if (!success) {
          ...
          await channel.send(`✅ Block succeeded (${pressed.size}/${req}).`).catch(() => {});
          for (const uid of nowAlive) {
            if (pressed.has(uid)) {
              const player = await getPlayer(uid);
              const mult = getEventMultiplier(boss.def.event, player);
      ...
      if (r.type === "quick_block" || r.type === "finisher") {
        ...
        for (const uid of nowAlive) {
          const player = await getPlayer(uid);
          const hasReverse = boss.def.event === "jjk" && player.jjk.items.reverse_talisman;
          ...
          if (pressed.has(uid)) {
            ...
            if (hasReverse && !boss.reverseUsed.has(uid)) {
              ...
              const nm = safeName(boss.participants.get(uid)?.displayName);
              await channel.send(`✨ **${nm}** was saved by Reverse Technique!`).catch(() => {});
            } else {
      ...
      if (r.type === "combo_defense") {
        ...
        const msg = await channel.send({
          content: `🎮 COMBO DEFENSE: ${Math.round((r.windowMs || 5000) / 1000)}s • ${seqText}`,
        ...
        if (msg?.id) {
          const disabled = comboDefenseRows(token, boss.def.id, i);
          disabled[0].components.forEach((b) => b.setDisabled(true));
      ...
      if (r.type === "group_final") {
        ...
        if (wins < required) {
          await channel.send(`❌ Not enough successes (${wins}/${required}). Everyone loses.`).catch(() => {});
          for (const uid of nowAlive) {
            await applyHit(uid, boss, channel, `was overwhelmed!`);
            eliminate(uid, boss);
            await sleep(100);
          }
        } else {
      ...
      if (r.type === "multi_press") {
        ...
        const customId = `boss_action:${boss.def.id}:${i}:${token}:multi:press`;
        const msg = await channel.send({
          content: `🛡️ BLOCK x${boss.activeAction.requiredPresses}: ${Math.round((r.windowMs || 10000) / 1000)}s`,
          components: singleActionRow(customId, r.buttonLabel || "Block", r.buttonEmoji || "🛡️", false),
        }).catch(() => null);
        ...
        for (const uid of nowAlive) {
          ...
          if (cnt >= (action?.requiredPresses || 3)) {
            ...
            await applyHit(uid, boss, channel, `failed to block enough times! (${cnt})`);
          }
          await sleep(140);
        }
      ...
      if (r.type === "choice_qte") {
        const token = `${Date.now()}_${Math.random().toString(16).slice(2)}`;
        boss.activeAction = { token, roundIndex: i, mode: "choice", choice: new Map() };
        ...
        const msg = await channel.send({
          content: `⚡ Choose: ${Math.round((r.windowMs || 3000) / 1000)}s`,
        ...
        if (r.afterText || r.afterMedia) {
          const e = new EmbedBuilder().setColor(0x2b2d31).setImage(r.afterMedia || boss.def.spawnMedia);
      ...
      if (r.type === "scripted_hit_all") {
        ...
        if (r.endMedia || r.endText) {
          const e = new EmbedBuilder().setColor(0x2b2d31).setImage(r.endMedia || boss.def.spawnMedia);
      ...
      if (r.type === "tri_press") {
        ...
        for (const uid of nowAlive) {
          ...
          if (ok) {
            ...
            await applyHit(uid, boss, channel, `couldn't regain focus!`);
          }
          await sleep(140);
        }
      ...
      if (r.type === "final_quiz") {
        const token = `${Date.now()}_${Math.random().toString(16).slice(2)}`;
        boss.activeAction = { token, roundIndex: i, mode: "quiz", choice: new Map() };
        ...
        const msg = await channel.send({
          content: `⏳ Answer: ${Math.round((r.windowMs || 8000) / 1000)}s`,
          components: triChoiceRow(btns, false),
        }).catch(() => null);
    ...
    for (const uid of survivors) {
      ...
      const winBase = boss.def.winRewardRange ? randInt(boss.def.winRewardRange.min, boss.def.winRewardRange.max) : boss.def.winReward;
      ...
      if (boss.def.event === "jjk" && boss.def.shardDropRange) {
        ...
        lines.push(`🧩 <@${uid}> got **${shards} Cursed Shards**.`);
      }
      if (boss.def.event === "jjk" && boss.def.expeditionKeyChance) {
        if (Math.random() < boss.def.expeditionKeyChance) {
          ...
          lines.push(`🗝️ <@${uid}> got **Expedition Key**!`);
        }
      }
      ...
      if (boss.def.roleDropId && Math.random() < chance) {
        ...
        lines.push(res.ok ? `🎭 <@${uid}> obtained a role!` : `⚠️ <@${uid}> role saved (bot couldn't assign).`);
      }
    }
    ...
    await channel.send("Boss event crashed.").catch(() => {});
  } finally {
...
async function spawnBoss(channel, bossId) {
  ...
  if (!isAllowedSpawnChannel(def.event, channel.id)) {
    await channel.send("Wrong channel for this boss.").catch(() => {});
    return;
  }
  ...
  if (def.preText) {
    ...
    if (def.teaserMedia) {
      const teaser = new EmbedBuilder().setColor(0x2b2d31).setImage(def.teaserMedia);
