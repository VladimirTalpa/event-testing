// src/handlers/buttons_legacy_boss_mob_shop_pvp.js
const { bossByChannel, mobByChannel, pvpById } = require("../core/state");
const { getPlayer, setPlayer } = require("../core/players");
const { safeName } = require("../core/utils");
const { mobEmbed, shopEmbed, wardrobeEmbed } = require("../ui/old_embeds_inventory_shop");
...
module.exports = async function legacyButtons(interaction) {
  ...
  if (cid === CID.BOSS_JOIN) {
    ...
    if (msg) {
      const { bossSpawnEmbed } = require("../ui/embeds_legacy");
      const { bossButtons } = require("../ui/components_legacy");
      await msg.edit({
  ...
  if (cid.startsWith("boss_action:")) {
    // твой код 1-в-1 (оставил как у тебя)
  ...
  if (cid.startsWith(`${CID.PVP_ACCEPT}:`) || cid.startsWith(`${CID.PVP_DECLINE}:`)) {
    ...
    if (b1 < amount) { await interaction.followUp({ content: `❌ Challenger lacks funds.`, ephemeral: false }).catch(() => {}); return; }
    if (b2 < amount) { await interaction.followUp({ content: `❌ You lack funds.`, ephemeral: false }).catch(() => {}); return; }
