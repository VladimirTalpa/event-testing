...
const { titlesEmbed } = require("../ui/embeds");
const { titlesComponents, CID } = require("../ui/components");
...
module.exports = async function handleSelects(interaction) {
  if (interaction.customId !== CID.TITLES_SELECT) return;
  ...
  if (!p.ownedRoles.includes(String(roleId))) {
    return interaction.reply({ content: "❌ This title is not in your collection.", ephemeral: true });
  }
  ...
  return interaction.update({
    ...
    components: titlesComponents(interaction.guild, updatedMember, p),
  });
};
